...
#include <stm32h7xx_ll_rcc.h>     /* LL_RCC_GetUSARTClockFreq() */
#include "lin_uart.h"
#include "lin.h"

extern LinInitStruct LinSettings;
void InitLinUartBaudrate(LinBaud br)
{
  ...
  if(LinSettings.Baudrate == BAUDRATE_9600)
    baudrate = 9600;
  else if(LinSettings.Baudrate == BAUDRATE_19200)
    baudrate = 19200;
  else /* Default value */
    baudrate = 0;
...
void LinUartIrqHandler(void)
{
  ...
  if (LL_USART_IsActiveFlag_RXNE_RXFNE(LIN_UART) && LL_USART_IsEnabledIT_RXNE_RXFNE(LIN_UART))
  {
