...
/*
 * Set LIN is transmitting flag and enable usart TXE flag
 */
...
/*
 * Slave received or sent some data callback (determined by the buffer direction
 * - tx or rx)
 */
...
/*
 * Initialize the actual sending of the slave response to the bus.
 * Parameter is the index to the structure with slave frame buffers (
 */
void SendSlaveResponse(uint8_t index);
/*
 * Reset the slave FSM (and zero the linIsReceiving variable).
 */
...
void LinErrorCallback(uint8_t linId, uint8_t error);
/*
 * Turn off the slave loopback control
 */
...
/*
 * Process LIN Frame to output message master request frame
 */
uint8_t FillMasterRequestLinFrame( uint8_t * pData, uint8_t dataLength, LIN_Frame* pFrame );

/*
 * Process the configuration data to actual configuration of LIN
 */
...
/*
 * Process LIN Frame to output message master response frame
 */
uint8_t FillMasterResponseLinFrame( uint8_t * pData, uint8_t dataLength, LIN_Frame* pFrame );
...
/*
 * Write LIN slave response to the buffer (or overwrite it).
 */
uint8_t WriteToLinSlaveResponseBuffer(uint8_t linId, uint8_t bufferDir,
                                      uint8_t length, uint8_t chksumType,
                                      uint8_t* pData);
