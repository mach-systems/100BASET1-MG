...
/* USER CODE BEGIN 1 */
uint8_t TakeHethAccess(void)
{
    return osMutexAcquire(hethMutexHandle, osWaitForever) == osOK;
}
void GiveAccessToHeth(void)
{
    osMutexRelease(hethMutexHandle);
}
/* USER CODE END 1 */
...
/* USER CODE BEGIN IP_ADDRESSES */
  /* Init Switch before LWIP, because it produces the ETH CLK for MCU */
  if (SjaInit(&hspi1))
     Error_Handler();

  /* Start LIN peripherals if they are configured in autoStart mode*/
  LinAutoStart();

  // Apply our configuration settings
  CONFIGURATION config = GetConfiguration();
  IP_ADDRESS[0] = config.IpAddress[0];
