...
extern uint8_t SJA1105P_forwardRecvFrames(void)
{
	...
	static uint8_t nPendingMetaFrames;
	...
	do
	{
		...
		{  /* This is a regular frame */
			...
			if (trapInformation.trapped == 1U)
			{
				...
				{
					if (nPendingMetaFrames < SJA1105P_N_SWITCHES)
					{  /* remember the frame while waiting for the meta frame */

						#if SJA1105P_ETHIF_ZEROCOPY == 0U
							for (i = 0; i < p_recvFrameDescriptor->len; i++)
							{
								frameBufWaitingMeta[nPendingMetaFrames][i] = p_frameBuf[i];
							}
							frameDescriptorWaitingMeta[nPendingMetaFrames] = *p_recvFrameDescriptor;
						#else  /* No copy, just maintain the pointer */
							p_frameBufWaitingMeta[nPendingMetaFrames] = p_frameBuf;
							p_frameDescriptorWaitingMeta[nPendingMetaFrames] = p_recvFrameDescriptor;
						#endif
						trapInformationWaitingMeta[nPendingMetaFrames] = trapInformation;
		...
		{  /* this frame is a meta frame */
			...
			nPendingMetaFrames--; /* meta frame received - no longer waiting for this one */

			#if SJA1105P_ETHIF_ZEROCOPY == 0U			
				ret += forwardTrappedFrame(&metaData, &trapInformationWaitingMeta[nPendingMetaFrames], &frameDescriptorWaitingMeta[nPendingMetaFrames], frameBufWaitingMeta[nPendingMetaFrames]);
			#else
				ret += forwardTrappedFrame(&metaData, &trapInformationWaitingMeta[nPendingMetaFrames], p_frameDescriptorWaitingMeta[nPendingMetaFrames], p_frameBufWaitingMeta[nPendingMetaFrames]);
			#endif
			if (g_forwardMeta == 1U)
